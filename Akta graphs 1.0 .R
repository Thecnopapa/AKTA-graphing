...
# Place this file on the folder with the excel files or above them
...
legend_x <- 0.95
legend_y <- 0.5
legend_justification <- c("right", "bottom") # For inside legends
legend_position <- "right" # For outside legends
main_colour <- "darkblue"
conductivity_colour <- "orange"
B_colour <- "green"

font_size <- 10
legend_font_size <- font_size/2
start_at_elution <- TRUE
calibrate_UV <- TRUE

use_fraction_lines <- TRUE
fraction_names_font_size <- 1
fraction_line_width <- 0.1
fraction_line_type <- "longdash"
...
title_just <- 0.5 # 0 = left, 0.5 = centre, 1 = right

n_right_axis_breaks <- 3
